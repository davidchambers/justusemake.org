...
module.exports = function recur(input, output, stack) {
  ...
  if (input === '') {
    ...
    return recur($[2], output + open('targets') + $[1].split(/([ ]+)/).map(function(s, idx) {
      return idx % 2 ? s : s && wrap(specialTargets.indexOf(s) >= 0 ? 'target special' : 'target', s);
    }).join('') + ':' + close(), stack);
    ...
    return functionNames.indexOf($[2]) >= 0 ?
      recur($[3], output + open('group') + $[1] + wrap('function', $[2]), concat(stack, [$[1]])) :
      recur($[2] + $[3], output + open('group') + $[1], concat(stack, [$[1]]));
  } else if (($ = /^([$][@%<?^+|*])([\s\S]*)$/.exec(input))) {
